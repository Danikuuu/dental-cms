...
Route::middleware(['auth'])->group(function () {
    ...
    // ── Patients ──────────────────────────────────────────────────────────
    Route::middleware('role:admin,dentist,receptionist,staff')->group(function () {
        Route::resource('patients', PatientController::class);
        Route::post('/patients/{patient}/images', [PatientImageController::class, 'store'])->name('patient-images.store');
        Route::get('/patient-images/{image}/file', [PatientImageController::class, 'show'])->name('patient-images.show');
    ...
    // ── Billing ───────────────────────────────────────────────────────────
    Route::middleware('role:admin,receptionist,staff')->prefix('billing')->name('invoices.')->group(function () {
    ...
    // ── Insurance ─────────────────────────────────────────────────────────
    Route::middleware('role:admin,receptionist,staff')->prefix('insurance')->name('insurance.')->group(function () {
    ...
    Route::middleware('role:admin')->prefix('inventory')->name('inventory.')->group(function () {
    ...
    Route::prefix('sms')->name('sms.')->group(function () {
        Route::middleware('role:admin,receptionist,staff')->group(function () {
            ...
            Route::post('/send', [SmsController::class, 'send'])->middleware('throttle:20,1')->name('send');
            Route::post('/bulk', [SmsController::class, 'bulkReminders'])->middleware('throttle:5,1')->name('bulk');
