...
class SmsController extends Controller
{
    ...
    public function bulkReminders(Request $request)
    {
        ...
        foreach ($appointments as $appt) {
            ...
            // Only send if it's due now; otherwise leave it queued as pending
            if ($reminder->scheduled_at && $reminder->scheduled_at->lessThanOrEqualTo(now())) {
                $this->dispatchSms($reminder);
                $sent++;
            }
        }
        ...
        if ($skipped) {
            $msg .= " {$skipped} patient(s) skipped (no phone number on file).";
        }
